...
fn test_default() {
    ...
    assert_eq!(
        context.brsa_blind_message_generate(
            &mut blind_msg,
            &mut msg,
            msg_len,
            &mut client_secret,
    ...
    assert_eq!(
        context.brsa_blind_sign(&mut blind_sig, &mut sk, &blind_msg),
        0
    );
...
fn test_deterministic() {
    ...
    assert_eq!(
        context.brsa_blind_sign(&mut blind_sig2, &mut sk, &blind_msg),
        0
    );
    blind_msg.brsa_blind_message_deinit();
    sk.brsa_secretkey_deinit();
