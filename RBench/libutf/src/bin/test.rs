...
fn test_utf8_convert() {
    fn test_convert(string: &[u16], expected: &[u8], written: usize) {
        ...
        assert_eq!(
            libutf_utf::utf16le_convert_to_utf8(string, &mut result),
            written
        );
    ...
    test_convert(
        &[0x4E67, 0x97F3, 0x4E2D],
        b"\xE6\x9D\x8E\xE9\x9F\xB3\xE4\xB8\xAD",
        9,
    );
...
fn test_utf16_convert() {
    fn test_convert(string: &[u8], expected: &[u16], written: usize) {
        ...
        assert_eq!(
            libutf_utf::utf8_convert_to_utf16le(string, &mut result),
            written
        );
        assert_eq!(&result[..written], expected);
    }
    ...
    test_convert(
        b"\x41\xCE\xA9\xE2\x98\x83\xE4\xB8\xAD\xF0\x9F\x98\x82",
        &[0x0041, 0x03A9, 0x2603, 0x4E2D, 0xD83D, 0xDE02],
        6,
    );
}
