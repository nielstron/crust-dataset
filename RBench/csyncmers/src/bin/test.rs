...
fn test() {
    ...
    for test in 1..=num_tests {
        ...
        let b = if b_max < b_min {
            b_min
        } else {
            rand::thread_rng().gen_range(b_min..=b_max)
        };

        println!(
            "Test {}: Sequence Length={}, K={}, S={}",
        ...
        compute_closed_syncmers_naive(
            &sequence,
            sequence.len(),
            a,
            b,
            &mut naive_results,
            &mut num_naive_results,
        );
        ...
        assert_eq!(
            ...
            "Mismatch in number of closed syncmers: {} (original) vs {} (naive)",
            num_results, num_naive_results
        );

        for (i, (result, naive_result)) in results.iter().zip(naive_results.iter()).enumerate() {
            assert_eq!(
                result.kmer_position, naive_result.kmer_position,
                "Mismatch at index {}: Position",
                i
            );
            assert_eq!(
                result.minimizer_hash, naive_result.minimizer_hash,
                "Mismatch at index {}: Hash",
                i
            );
        }

        println!(
            "Test {} passed. All closed syncmers match between original and naive method.\n",
            test
        );
    }
