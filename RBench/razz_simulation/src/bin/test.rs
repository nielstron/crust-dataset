...
fn test_sort_card_by_rank_1(len: u64, pos: u64, c: &Option<Card>) -> ItrAction {
    ...
    assert_eq!(
        c.as_ref().unwrap().get_card_suit_rank(),
        test_data[pos as usize]
    );
    ItrAction::Continue
}
...
fn test_card_operations() {
    ...
    d.as_mut()
        .unwrap()
        .strip_card_from_deck(CardSuitRank::HeartK);
    assert!(d.as_ref().unwrap().is_card_in_deck(CardSuitRank::HeartK) == 0);
    d.as_mut()
        .unwrap()
        .strip_card_from_deck(CardSuitRank::Heart9);
    ...
    for _ in 1..=50 {
        ...
        assert!(
            d.as_ref()
                .unwrap()
                .is_card_in_deck(c.as_ref().unwrap().get_card_suit_rank())
                == 0
        );
    }
    c = d.as_mut().unwrap().deal_from_deck();
    ...
    assert_eq!(
        h.as_ref().unwrap().get_max_rank_of_hand(),
        CardRank::InvalidRank
    );
    h.as_mut()
        .unwrap()
        .insert_into_hand(&d.as_mut().unwrap().deal_from_deck());
    ...
    let expected_card_count = [
        1001, 1012, 1032, 911, 1000, 987, 1026, 978, 971, 964, 942, 968, 1033, 962, 1011, 939, 975,
        1052, 1028, 1002, 992, 1054, 1004, 1045, 993, 984, 1023, 960, 993, 1058, 998, 971, 1018,
        1025, 1042, 1045, 983, 1018, 999, 995, 1024, 999, 968, 1024, 986, 1015, 1036, 1015, 966,
        973, 982, 1018,
    ];
