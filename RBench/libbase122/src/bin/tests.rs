...
fn bitstring_to_bytes(bitstring: &str) -> Vec<u8> {
    ...
    let chunks: Vec<&str> = clean_bitstring
        .as_bytes()
        .chunks(8)
    ...
    // Two byte test
    {
        ...
        }
        {
            ...
            assert_eq!(
                output.clone(),
                [0x87, 0x80],
                "Expected output to be [0x87, 0x80]"
            );
        }
        {
            let mut writer = BitWriter::new(Some(&mut output), 2);
        ...
        }
        {
            ...
            assert_eq!(
                output.clone(),
                [0x87, 0xA0],
                "Expected output to be [0x87, 0xA0]"
            );
        }
        {
            let mut writer = BitWriter::new(Some(&mut output), 2);
            ...
            assert_eq!(
                output.clone(),
                [0x87, 0xBF],
                "Expected output to be [0x87, 0xBF]"
            );
        }
        {
            let mut writer = BitWriter::new(Some(&mut output), 2);
            ...
            assert!(
                result.is_err(),
                "Expected write to fail due to insufficient space"
            );
        }
    }
}
...
fn test_decode_error_cases() {
    ...
    for (encoded, expected_error) in test_cases.iter() {
        ...
        assert!(
            result.is_err(),
            "Expected decode to fail for input: {}",
            encoded
        );
...
fn test_decode_success_cases() {
    ...
    for (encoded, expected) in test_cases.iter() {
        ...
        assert!(
            result.is_ok(),
            "Expected decode to succeed for input: {}",
            encoded
        );
        ...
        assert_eq!(
            ...
            "Decoded output does not match expected for input: {}",
            encoded
        );
    }
}
...
fn test_roundtrip() {
    let tests = [
        ...
        (
            "illegal one byte",
            "00000000 11111111",
            "11000010 10111111 01100000",
        ),
        (
            "illegal last two bits",
            "11111111 11111100",
            "01111111 01111111 11011110 10000000",
        ),
    ...
    for (description, data, encoded_expected) in tests.iter() {
        ...
        assert_eq!(
            ...
            "Encoded output does not match expected for test: {}",
            description
        );
        ...
        assert_eq!(
            decoded, input,
            "Decoded output does not match input for test: {}",
            description
        );
    }
}
