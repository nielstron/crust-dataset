use mdb::mdb::{Mdb, MdbError, MdbOptions, MdbStatusCode};
...
fn happy_test0() {
    ...
    db.write("misakawa", "mikoto")
        .expect("Failed to write key-value");

    // Read the value back
    let mut buffer = vec![0u8; 257];
    let read_size = db
        .read("misakawa", &mut buffer)
...
fn reopen_test1() {
    ...
    // Create database and write data
    {
        ...
        db.write("Lisp", "LambdaExpression")
            .expect("Failed to write key-value");
